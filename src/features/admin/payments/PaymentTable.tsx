...
export default function PaymentTable({
  ...
  const isDefaultQuery =
    activeTab === "pending" && debouncedSearch === "" && page === 1;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-zinc-900/60 p-3 sm:p-4 rounded-2xl border border-amber-500/20">
        <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 scrollbar-none">
          ...
          {[
            ...
            { id: "paid", label: "تایید شده", count: currentStats.paidCount },
            { id: "failed", label: "رد شده", count: currentStats.failedCount },
      ...
      <div className="bg-zinc-900/80 border border-amber-500/20 rounded-3xl overflow-hidden shadow-2xl">
              ...
              return (
                ...
                >
                  <div className="space-y-3 flex-1">
                    ...
                    {order.paymentRef && (
                      <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 flex items-center justify-between gap-3 min-w-0">
                        <div className="flex items-center gap-2 min-w-0 overflow-hidden">
                          <span className="text-xs text-amber-300 font-medium shrink-0">
                            کد پیگیری / شناسه واریز:
                          </span>
                          <span className="font-mono font-bold text-amber-200 dir-ltr text-sm select-all truncate break-all">
                            {order.paymentRef}
                          </span>
                        </div>
                        <button
                          ...
                          className="p-1.5 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 rounded-lg transition-colors flex items-center gap-1 text-xs cursor-pointer shrink-0"
                        >
        ...
        {Boolean(data?.totalPages && data.totalPages > 1) && (
          <div className="p-4 bg-black/40 border-t border-amber-500/10">
            <AppPagination
              currentPage={page}
